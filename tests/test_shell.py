...
from tools.shell import _shell_impl, register_shell_tools
...
class TestShellTool(unittest.IsolatedAsyncioTestCase):
    ...
    # ---------------------------------------------------------
    # GROUP 1: Validation & Execution (_shell_impl)
    # ---------------------------------------------------------

    async def test_missing_command_argument(self):
        """Test 1.1: _shell_impl returns ToolFailure if 'command' is missing."""
        result = await _shell_impl({}, self.ctx)
    ...
    async def test_empty_string_command(self):
        """Test 1.2: _shell_impl returns ToolFailure if 'command' is empty/whitespace."""
        result = await _shell_impl({"command": "   "}, self.ctx)
        self.assertIsInstance(result, ToolFailure)
        self.assertIn("command is required", result.error_message)

    async def test_default_timeout_parsing(self):
        """Test 1.3: _shell_impl defaults timeout to 120.0 seconds."""
        result = await _shell_impl({"command": "ls"}, self.ctx)
    ...
    async def test_explicit_timeout_parsing(self):
        """Test 1.4: _shell_impl correctly converts ms timeout to seconds."""
        result = await _shell_impl({"command": "sleep 5", "timeout": 5000}, self.ctx)
        self.assertIsInstance(result, ShellCallback)
    ...
    async def test_optional_description_parsing(self):
        """Test 1.5: _shell_impl captures the optional description field."""
        result = await _shell_impl({
    ...
    def test_registry_binding(self):
        ...
        register_shell_tools(mock_registry, self.ctx)
    ...
    def test_json_schema_structure(self):
        ...
        self.assertIn("BE CAREFUL! THERE ARE MANY DANGEROUS SHELL COMMANDS", description)
